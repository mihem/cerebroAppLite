...
getEnrichedPathways <- function(
  ...
  object@misc[["enriched_pathways"]][[paste0(
    marker_genes_input,
    "_enrichr"
  )]] <- list()
  ...
  for (i in seq_along(groups)) {
    ...
    ## get input data
    current_marker_genes <- object@misc$marker_genes[[marker_genes_input]][[
      current_group
    ]]
    ...
    if (
      ...
      message(
        paste0(
          '[',
          format(Sys.time(), '%H:%M:%S'),
          '] Get enriched pathways for group `',
          current_group,
          '`...'
        )
      )
      ...
      if (is.factor(current_marker_genes[[current_group]])) {
        ...
        group_levels <- group_levels[
          group_levels %in% current_marker_genes[[current_group]]
        ]
      ...
      if (length(results) == 0) {
        message(
          paste0(
            '[',
            format(Sys.time(), '%H:%M:%S'),
            '] 0 pathways passed the ',
            'threshold across all group levels and databases for group `',
            current_group,
            '`.'
          )
        )
        ...
        group_levels_with_results <- group_levels[
          group_levels %in% results[[current_group]]
        ]
        results[[current_group]] <- factor(
