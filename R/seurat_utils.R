...
.getSpatialData <- function(
  ...
  if (inherits(image_obj, "VisiumV1") || inherits(image_obj, "VisiumV2")) {
    ...
    coords <- try(
      Seurat::GetTissueCoordinates(image_obj, which = "centroids"),
      silent = TRUE
    )
    ...
    if (
      (inherits(coords, "try-error") || is.null(coords)) &&
        !is.null(image_obj@coordinates)
    ) {
  ...
  if (length(common_cells) == 0) {
    stop(
      "No common cells found between spatial coordinates and expression data."
    )
  }
