...
#' @param max_request_size Max upload size in MB; defaults to 200. Kept
#'   conservative so a generated bundle is safe to serve publicly without an
#'   8 GB upload becoming an out-of-memory / disk vector. Raise it explicitly
#'   for trusted local/lab deployments that ingest large \code{.crb} files.
...
#'   the generated app.
#' @param overwrite If \code{TRUE}, replace an existing \code{result_dir} that
#'   was previously generated by \code{createShinyApp} (identified by an
#'   \code{app.R} or \code{.cerebro_bundle} marker) or is empty. Defaults to
#'   \code{FALSE}. Protected paths (root, home, working dir, the parent of the
#'   target) and non-bundle directories are never deleted.
...
createShinyApp <- function(
  ...
  marker_file <- file.path(result_dir, ".cerebro_bundle")

  if (dir.exists(result_dir)) {
    if (!overwrite) {
      stop(
        "'result_dir' already exists: ",
        result_dir,
        "\n",
        "Refusing to touch it. Set overwrite = TRUE to replace a bundle ",
        "previously generated by createShinyApp(), or choose a fresh path.",
        call. = FALSE
      )
    }
    ## Guard the destructive unlink(): never wipe a protected directory, and
    ## only delete a directory that is empty or is a bundle we generated.
    real_target <- normalizePath(result_dir, mustWork = TRUE)
    protected <- unique(normalizePath(
      c("/", "~", getwd(), dirname(real_target)),
      mustWork = FALSE
    ))
    if (real_target %in% protected) {
      stop(
        "Refusing to overwrite a protected directory: ",
        real_target,
        call. = FALSE
      )
    }
    is_our_bundle <- file.exists(marker_file) || file.exists(app_file)
    is_empty <- length(
      list.files(real_target, all.files = TRUE, no.. = TRUE)
    ) ==
      0
    if (!is_our_bundle && !is_empty) {
      stop(
        "'result_dir' exists but is not a createShinyApp() bundle ",
        "(no app.R / .cerebro_bundle marker): ",
        real_target,
        "\n",
        "Refusing to delete it. Point result_dir at a fresh path.",
        call. = FALSE
      )
    }
    if (verbose) {
      cat("Removing existing bundle:", real_target, "\n")
    }
    unlink(real_target, recursive = TRUE, force = TRUE)
  }
  ...
  ## Marker so a later overwrite can recognise this as our bundle (S4 guard).
  ...
  app_content <- glue::glue(
    ...
    ## Security: the bundled data/ directory is deliberately NOT registered as a
    ## Shiny resource path. The .crb/H5 files are read server-side from disk and
    ## spatial images are base64-embedded, so no static URL is needed. Exposing
    ## data/ would let anyone download the raw datasets via /data/<file>.
