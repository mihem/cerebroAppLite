  ...
  fluidRow(
    cerebroBox(
      title = tagList(
        ...
        cerebroInfoButton("spatial_details_selected_cells_table_info")
      ),
      tagList(
        shinyWidgets::materialSwitch(
          inputId = "spatial_details_selected_cells_table_number_formatting",
        ...
        shinyWidgets::materialSwitch(
          inputId = "spatial_details_selected_cells_table_color_highlighting",
          label = "Highlight values with colors:",
          value = TRUE,
          status = "primary",
          inline = TRUE
        ),
        DT::dataTableOutput("spatial_details_selected_cells_table")
      )
    )
  )
})
...
## Text in info box.
##----------------------------------------------------------------------------##
spatial_details_selected_cells_table_info <- list(
  ...
  text = HTML(
    ...
    Similar to the automatic formatting option, when activated, CerebroNexus will look for known columns in the table (those that contain grouping variables), try to interpret column content, and use colors and other stylistic elements to facilitate quick interpretation of the values. If you prefer the table without colors and/or the identification does not work properly, you can simply deactivate this feature.<br>
    <em>This feature does not work on columns that contain 'NA' values.</em><br>
    <br>
