...
per-cell clonotype annotations. CerebroNexus lists it under `Suggests`, so
...
The module provides 21 tabbed visualizations. Each includes a contextual help
...
- **Definition**: a clone-definition resolution waterfall counting unique
  entities at each level (cells, V, J, V+J, CDR3, V+CDR3, V+J+CDR3); the drop
  from cells to V+J+CDR3 shows how much a stricter definition collapses the
  repertoire. Optionally faceted by the active group column. For BCR chains a
  caveat notes that CDR3 is not collapsed by somatic hypermutation
