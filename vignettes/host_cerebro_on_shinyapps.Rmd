...
Of course, we also need the cerebroAppLite package.

```{r eval=FALSE}
if ( 'rsconnect' %in% installed.packages() == FALSE ) install.packages('rsconnect')
if ( 'cerebroAppLite' %in% installed.packages() == FALSE ) install.packages('mihem/cerebroAppLite')
...
Next, we need to copy the Shiny files and some other data for Cerebro to a dedicated directory, here `~/test_cerebro_shinyapps`.
...
file.copy(
  system.file('shiny', package = 'cerebroAppLite'),
...
Cerebro.options <<- list(
  "mode" = "open",
  "crb_file_to_load" = "extdata/example.crb",
  "cerebro_root" = "."
)
...
When you run the `deployApp()` command, you might receive a warning about a hidden `browser()` call in the `shiny/about/server.R` file, but you can ignore that and proceed.
