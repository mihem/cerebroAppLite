...
Make sure that the `Matrix`, `HDF5Array` and `cerebroAppLite` packages are installed.
...
if ( 'cerebroAppLite' %in% installed.packages() == FALSE ) install.packages('cerebroAppLite')
...
Here, we use the example data set in the `cerebroAppLite` package:

```{r}
crb_input <- system.file('extdata', 'example.crb', package = 'cerebroAppLite')
