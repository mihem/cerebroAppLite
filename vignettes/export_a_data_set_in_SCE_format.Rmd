...
Before starting the workflow, we need to install `cerebroAppLite`, as well as the [`scran`](https://bioconductor.org/packages/release/bioc/html/scran.html), [`scater`](https://bioconductor.org/packages/3.11/bioc/html/scater.html) and [`SingleR`](https://www.bioconductor.org/packages/release/bioc/html/SingleR.html) packages, which are not installed as dependencies of cerebroAppLite because they are only necessary if you want/need to pre-process your scRNA-seq data.
...
if ( 'cerebroAppLite' %in% installed.packages() == FALSE ) install('mihem/cerebroAppLite')
...
sce@metadata$technical_info$cerebroApp_version <- utils::packageVersion('cerebroAppLite')
...
Finally, we use the `exportFromSeurat()` function of cerebroAppLite to export our Seurat object to a `.crb` file which can be loaded into Cerebro.
