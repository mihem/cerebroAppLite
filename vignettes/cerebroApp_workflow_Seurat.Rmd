...
```{r pre_processing, warning=FALSE, eval=FALSE}
...
```{r dimensional_reduction, warning=FALSE, eval=FALSE}
seurat <- RunUMAP(
  seurat,
  reduction.name = 'UMAP',
  reduction.key = 'UMAP_',
...
```{r prepare_meta_data, eval=FALSE}
...
```{r cell_type_assignment, eval=FALSE}
...
```{r build_phylogenetic_tree_for_clusters, eval=FALSE}
...
```{r build_phylogenetic_tree_for_cell_types, eval=FALSE}
...
```{r getMostExpressedGenes, eval=FALSE}
...
```{r getEnrichedPathways, eval=FALSE}
...
```{r analysis_with_monocle, eval=FALSE}
